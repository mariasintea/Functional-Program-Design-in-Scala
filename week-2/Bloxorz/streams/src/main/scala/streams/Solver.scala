...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    b.legalNeighbors.to(LazyList).map(neighbour => (neighbour._1, neighbour._2 :: history))
  ...
  def from(initial: LazyList[(Block, List[Move])], explored: Set[Block]): LazyList[(Block, List[Move])] = {
    if (initial.isEmpty)
      ...
      initial.head #:: from(initial.tail ++ neighbours, explored ++ neighbours.map(_._1))
    }
  }
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter(path => done(path._1))
  ...
  lazy val solution: List[Move] =
    if (pathsToGoal.isEmpty)
      ...
      pathsToGoal.map(path => path._2).head.reverse
}
